...
export default function DiscoverPage() {
  ...
  const {
    ...
    img,
    budgetType,
  } = location1.state || {}; // получаем пропсы из состояния

  return (
    <div
      className={
        type === "needs"
          ? discoverStyles["wrapper_bg-blue"]
          : discoverStyles.wrapper
      }
    >
      <div className={styles.needspage}>
        <section>
          <Link to={`/discover/${type}`}>
            <div className={styles.discover__back}>
          ...
          {option === "products" && img ? (
            <div className={styles["needspage__product-top"]}>
              ...
              <div style={isTablet ? { width: "100%" } : {}}>
                <div className={discoverStyles.discover__heading}>
                  {heading}
                </div>
                <DiscoverCard
        ...
        {isTablet && (
          <div className={styles["needspage__contact-mobile"]}>
            <div
              className={
                styles["main-button"] + " " + styles["card__view-more"]
              }
